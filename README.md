...
由於部屬聊天機器人涉及到多個專業領域的知識與技術，因此開發者需要掌握多種程式語言及專業技術，才有能力完成。而Asar平台的出現大幅降低了技術門檻。

Asar採用容器化的設計，實現快速部署。Asar使用Transformer、ALBERT近代深度學習自然語言處理技術（NLP），使聊天機器人具備自然語言理解（NLU）的能力。Asar提供專屬的視覺化設計工具，讓開發者能以流程圖的思維來設計聊天劇本、以拖拉程式方塊的方式來控制樹莓派的周邊設備。此外，Asar也提供了各大通訊平台的接口，簡化將聊天機器人整合至聊天室的步驟。受益於樹莓派的自由度、擴充性等優勢，Asar能夠應用於各種場景。
